...
| `/lab status` | `brainstorming`, `development` | `/lab status` | `scripts/lab` | none | `--json` | `no-write` | human-readable status report on stdout by default; JSON object on stdout with --json; `0` success, `1` runtime or validation failure, `2` usage, mode, or registry error |
| `/lab doctor` | `brainstorming` | `/lab doctor [<idea-id>]` | `scripts/lab` | none | `--idea-id`, `--json` | `no-write` | human-readable status report on stdout by default; JSON object on stdout with --json; `0` success, `1` runtime or validation failure, `2` usage, mode, or registry error |
...
### `/lab capture <idea-id>`
- Add/update idea in `ideas/_inbox.md` using `.harness/brainstorming/templates/idea_template.md`.
...
- Record decision in session using `.harness/brainstorming/templates/decision_template.md`.
- For major strategic changes, create ADR from `.harness/brainstorming/docs/adr/template.md`.
...
### `/lab risk <idea-id>`
- Record risk in session using `.harness/brainstorming/templates/risk_template.md`.
- Example:
...
### `/lab review <idea-id>`
- Record review notes and optional gate using `.harness/brainstorming/templates/review_gate_template.md`.
- Update `IDEA_CATALOG.md`.
- Example:
  - `./scripts/lab review --idea-id idea-template-hardening --result conditional-pass --summary "Ready after validator parity"`

### `/lab export <idea-id>`
- Optionally create `exports/YYYY-MM-DD_PROJECT_SUMMARY_<idea-id>.md` using `.harness/brainstorming/templates/project_plan_packet_template.md`.
...
- Optional repeated note:
  - `--note "<extra context>"`
- Example:
  - `./scripts/lab evidence --task "Tests pass" --command "python3 -m unittest discover -s .harness/tests -v" --result "60 tests passed"`
