...
class DevelopmentAdrTests(LabWorkflowTestCase):
    def test_lab_adr_creates_next_sequential_adr_in_development_mode(self) -> None:
        ...
        expected_path = self.repo / "docs/adr/ADR-0002-adopt-agent-harness-command-routing-defaults.md"
        self.assertTrue(expected_path.exists())
        adr = expected_path.read_text(encoding="utf-8")
        self.assertIn("Created docs/adr/ADR-0002-adopt-agent-harness-command-routing-defaults.md", result.stdout)
        self.assertIn('# ADR-0002: Adopt "Agent Harness" command routing / defaults', adr)
