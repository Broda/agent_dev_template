...
def _validate_powershell_launcher(root: Path, result: ValidationResult, script_name: str, cli_command: str) -> None:
    ...
    required_snippets = [
        ...
        'py -3 "$scriptDir/../.harness/runtime/python/cli.py"',
        f" {cli_command}",
        'python "$scriptDir/../.harness/runtime/python/cli.py"',
    ]
    for snippet in required_snippets:
        if snippet not in text:
...
def _validate_release_readiness_workflow(root: Path, result: ValidationResult) -> None:
    ...
    required_snippets = {
        ...
        "full unit suite": "python3 -m unittest discover -s .harness/tests -v",
        "plugin package smoke": "python3 .harness/plugins/project-lifecycle-lab/smoke_package.py .harness/plugins/project-lifecycle-lab",
