...
def validate_notes_catalog(root: Path, result: ValidationResult, *, base: str = "") -> None:
    notes_catalog = f"{base}/NOTES_CATALOG.md" if base else "NOTES_CATALOG.md"
    notes_prefix = f"{base}/notes/" if base else "notes/"
    notes_catalog_path = root / notes_catalog
    if not notes_catalog_path.exists():
        result.add_failure(f"Missing {notes_catalog}")
        return
    ...
    for cells in parse_markdown_table_rows(notes_catalog_path, re.compile(r"^\|\s*note-\d{4}\s*\|")):
        ...
        if not clean_note_path.startswith(notes_prefix):
            result.add_failure(f"Note path for '{note_id}' must be under {notes_prefix}: {clean_note_path}")
...
def validate_template_cli_file_map(root: Path, result: ValidationResult) -> None:
    ...
    module_root = root / ".harness/runtime/python/template_cli"
