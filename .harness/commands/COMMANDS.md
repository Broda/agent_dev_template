...
| `/lab decide` | `brainstorming` | `/lab decide <decision-slug>` | `scripts/lab` | `--idea-id` | `--decision-id`, `--owner`, `--session`, `--decision-level`, `--situation`, `--chosen-option`, `--rationale`, `--constraints`, `--no-sync` | `write` | human-readable command result on stdout; `0` success, `1` runtime or validation failure, `2` usage, mode, or registry error |
| `/lab risk` | `brainstorming` | `/lab risk <idea-id>` | `scripts/lab` | `--idea-id` | `--risk-id`, `--owner`, `--session`, `--statement`, `--mitigation`, `--contingency`, `--probability`, `--impact`, `--no-sync` | `write` | human-readable command result on stdout; `0` success, `1` runtime or validation failure, `2` usage, mode, or registry error |
