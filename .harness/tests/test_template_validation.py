...
class TemplateValidationTests(LabWorkflowTestCase):
    ...
    def test_validate_brainstorming_checks_powershell_launcher_delegation(self) -> None:
        ...
        launcher_path.write_text(
            launcher_path.read_text(encoding="utf-8").replace(
                'py -3 "$scriptDir/../.harness/runtime/python/cli.py"', "py -3 legacy.ps1"
            ),
            encoding="utf-8",
        )
    ...
    def test_validate_brainstorming_checks_release_readiness_workflow_coverage(self) -> None:
        ...
        workflow_path.write_text(
            workflow_path.read_text(encoding="utf-8").replace(
                "python3 -m unittest discover -s .harness/tests -v", "", 1
            ),
            encoding="utf-8",
        )
    ...
    def test_validate_brainstorming_checks_template_cli_file_map_rows(self) -> None:
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn(
            "FILE_MAP.md missing registry row for template CLI module: .harness/runtime/python/template_cli/workflow_render.py",
            result.stdout,
        )

    def test_validate_brainstorming_checks_finalization_overwrite_policy_paths(self) -> None:
        policy_path = self.repo / ".harness/commands/finalization_overwrite_policy.json"
    ...
    def test_validate_brainstorming_checks_note_metadata_matches_catalog(self) -> None:
        note_path = self.repo / "notes/2026-04-05_note-0001-example-research-note.md"
        note_path.write_text(
            "# Research Note\n\n## Metadata\n\n- Note ID: note-9999\n- Title: Wrong note title\n"
            "- Date: 2026-04-06\n- Related Idea ID: n/a\n- Source Context: example discussion\n- Tags: wrong-tag\n",
            encoding="utf-8",
        )
        catalog_path = self.repo / "NOTES_CATALOG.md"
        catalog_path.write_text(
            catalog_path.read_text(encoding="utf-8")
            + "| note-0001 | Example research note | 2026-04-05 | n/a | example discussion "
            "| `notes/2026-04-05_note-0001-example-research-note.md` | example,research |\n",
            encoding="utf-8",
        )
        ...
        self.assertIn("Date is '2026-04-06', expected '2026-04-05'", result.stdout)
        self.assertIn("Tags is 'wrong-tag'", result.stdout)

    def test_validate_brainstorming_checks_python_file_size(self) -> None:
        oversized_path = self.repo / ".harness/tests/oversized_fixture.py"
        oversized_path.write_text("\n".join("x = 1" for _ in range(351)) + "\n", encoding="utf-8")

        result = run_cmd(["./scripts/validate-brainstorming"], cwd=self.repo, check=False)

        self.assertNotEqual(result.returncode, 0)
        self.assertIn("Python code file exceeds 350 lines: .harness/tests/oversized_fixture.py (351)", result.stdout)
    ...
    def test_validate_brainstorming_checks_workflow_finalize_helper_imports(self) -> None:
        workflow_path = self.repo / ".harness/runtime/python/template_cli/workflow.py"
        workflow_path.write_text(
            workflow_path.read_text(encoding="utf-8").replace(
