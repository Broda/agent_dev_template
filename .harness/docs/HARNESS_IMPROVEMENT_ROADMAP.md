...
- [x] Decide whether `.github/workflows/governance-audit.yml` should be removed
      ...
      by blocking CI.
- [x] Update `README.md`, `.harness/brainstorming/FILE_MAP.md`, and validation fixtures
      whenever public artifact inventory changes.

Exit criteria:

- [x] `./scripts/validate-governance` passes.
- [x] `python3 -m unittest discover -s .harness/tests -v` passes.
...
- [x] Add tests proving `project-harness new` stamps or preserves provenance
      correctly.
- [x] Document the manifest in `README.md` and `.harness/docs/BOOTSTRAP_TOOL.md`.
...
- [x] Decide whether the capability manifest extends
      `.harness/commands/intent_registry.json` or lives as a separate generated
      file.
...
- [x] Render human docs from the same source so
      ...
      `.harness/commands/CONVERSATIONAL_MODE.md` stay secondary views.
...
- [x] Add regression tests for clean project, locally modified wrapper,
      ...
      scenarios.
- [x] Document the update boundary in `.harness/docs/BOOTSTRAP_TOOL.md`.
...
- [x] Preserve local fallback behavior for users who do not install a global
      runtime.
- [x] Identify which copied `.harness/runtime/python/template_cli` modules can stop being
...
- [x] Make `.harness/docs/HARNESS_IMPROVEMENT_ROADMAP.md` updates part of template-maintenance
...
- [x] Consolidate harness-owned files under `.harness/` while preserving stable
      root entrypoints and active brainstorming history.
  - Moved command contracts, phase docs/templates, harness docs/ADRs, plugin
    package mirrors, Python runtime code, and harness tests under `.harness/`.
  - Kept `./scripts/*`, `AGENTS.md`, `MODE.md`, `.agents/skills/`, and active
    brainstorming folders (`ideas/`, `sessions/`, `notes/`, `exports/`) at the
    root for compatibility and day-to-day use.
  - Finalization now archives brainstorming history into `.harness/history/`
...
- [ ] If pursuing the 300-line cap, split the remaining largest test/helper
      modules first.
  - Candidate files from the survey: `.harness/tests/test_finalization_regression.py`,
    `.harness/tests/test_development_rendering.py`,
    `.harness/tests/test_intent_registry_contract.py`, `.harness/tests/workflow_test_helpers.py`,
    and `.harness/tests/test_template_validation.py`.
...
- [x] Update `.harness/commands/harness_manifest.json` stable wrapper metadata so
...
- [x] Add JSON Schema files for `.harness/commands/harness_manifest.json` and
...
- [ ] Continue splitting orchestration-heavy modules when cohesive boundaries are
  ...
  - Acceptance: every split preserves public imports or adds compatibility
    exports where needed, updates `.harness/brainstorming/FILE_MAP.md`, updates retained
